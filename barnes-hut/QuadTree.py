...
class QuadTree:
	MAX_DEPTH = 10 # TODO: implement max depth
	# EMPTY_CHILDREN = [None]
	class _Node:
		def __init__(self, parent, children, center, width, body):
			self._parent = parent
	...
	def __init__(self, width):
		self._root = QuadTree._Node(
			...
			(width / 2, width / 2),
			width,
			Body(0, 0, [width / 2, width / 2], [0, 0])
		)
		self._width = width

	def is_empty(self):
		return self._root._children == [None]*4

	def _recursive_insert(self, node, body):
		x, y = body.pos
		node_x, node_y = node._center
		right = x >= node_x
		top = y >= node_y
		if x > (node_x + node._width / 2) or y > (node_y + node._width / 2) or x < 0 or y < 0:
			#TODO: what do we do then?

		if not right and top:
			'''top left'''
			if node._children['topleft'] is None:
				node._children['topleft'] = QuadTree._Node(
					node,
	...
	def _rec_insert(self, node, body):
		...
		if not right and top:
			...
			if node._children[0] is None:
				node._children[0] = QuadTree._Node(
					node,
					[None]*4,
					(0,self.topleft),
					(centerx, centery),
